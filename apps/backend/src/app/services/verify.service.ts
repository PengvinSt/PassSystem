...
export class VerifyService {
  ...
  public async verify(body: {uuid:string, token:string}): Promise<IActionStatus> {
    const candidate = await this.userModel.findOne({uuid:body.uuid}).exec();
    if(!candidate)  throw new HttpException(`User is not exist!`, HttpStatus.BAD_REQUEST);
  ...
  public async userOnline(body: {uuid:string, isVerified:boolean}): Promise<IActionStatus> {
    const candidate = await this.userModel.findOne({uuid:body.uuid}).exec();
    if(!candidate)  throw new HttpException(`User is not exist!`, HttpStatus.BAD_REQUEST);
    ...
    await this.userModel.findOneAndUpdate({uuid:body.uuid}, newUser, {new: true}).exec()

    return {message:'User set to online', success:true}
  }

  public async userOffline(body: {uuid:string, isVerified:boolean}): Promise<IActionStatus> {
    const candidate = await this.userModel.findOne({uuid:body.uuid}).exec();
    if(!candidate)  throw new HttpException(`User is not exist!`, HttpStatus.BAD_REQUEST);
