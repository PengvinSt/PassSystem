...
export class VerifyController {
  ...
  @UseGuards(AuthGuard())
  verifyUser(@Body() body: {uuid:string, token:string}): Promise<IActionStatus> {
 ...
 // @ApiParam({ name: "uId", required: true, description: "User identifier" })
  ...
  @UseGuards(AuthGuard())
  setUserOnline( @Body() body: {uuid:string, isVerified:boolean}): Promise<IActionStatus> {
  ...
  @ApiResponse({ status: HttpStatus.BAD_REQUEST, description: "Bad Request" })
  @UseGuards(AuthGuard())
  setUserOffline(@Body() body: {uuid:string, isVerified:boolean}): Promise<IActionStatus> {
